...
function formatRateLimitSnapshotSummary(snapshot: UiRateLimitSnapshot): string {
  ...
  if (snapshot.credits?.unlimited) return t('Unlimited credits')
  if (snapshot.credits?.hasCredits && snapshot.credits.balance) {
    return `${snapshot.credits.balance} ${t('credits')}`
  }
  return ''
}
...
function formatActiveSparkSummary(account: UiAccountEntry): string {
  ...
  return summary ? `${t('Spark')}: ${summary}` : ''
}
...
function formatAccountQuota(account: UiAccountEntry): string {
  if (isAccountUnavailable(account)) {
    return appendSparkSummary(account.quotaError || t('402 Payment Required'), account)
  }
  const quota = account.quotaSnapshot
  ...
  if (quota?.credits?.unlimited) {
    return appendSparkSummary(t('Unlimited credits'), account)
  }
  if (quota?.credits?.hasCredits && quota.credits.balance) {
    return appendSparkSummary(`${quota.credits.balance} ${t('credits')}`, account)
  }
  if (account.quotaStatus === 'loading') {
    return appendSparkSummary(t('Loading quota…'), account)
  }
  if (account.quotaStatus === 'error') {
    return appendSparkSummary(account.quotaError || t('Quota unavailable'), account)
  }
  if (account.quotaStatus === 'ready' || account.quotaStatus === 'idle') {
    return appendSparkSummary(t('Quota unavailable'), account)
  }
  return appendSparkSummary(t('Fetching account details…'), account)
}
